...
function initFaceMesh() {
  ...
  readingCamera = new Camera(readingVideo, {
    onFrame: async () => { await faceMesh.send({ image: readingVideo }); },
    width: 240,
    height: 180
  });

  document.getElementById('reading-text-panel').addEventListener('scroll', (e) => {
...
function updateReadingProgressUI() {
  ...
  document.getElementById('scroll-progress').textContent = `${Math.round(maxScrollProgress)}%`;
...
function startReadingSession() {
  ...
  const finalText = rawText.length > 0
    ? rawText
...
function finishReadingSession() {
  ...
  const attentionPct = totalSamples === 0 ? 0 : (attentiveSamples / totalSamples) * 100;
  const verifiedPct = Math.round((maxScrollProgress / 100) * (attentionPct / 100) * 100);
